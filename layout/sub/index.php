...
if ($config['UseChangelogTicker']) {
	...
	if (isset($changelogs) && !empty($changelogs) && $changelogs !== false) {
		...
		<div class="block-title">Changelogs</div>
			
		<div style="border: 1px solid gray"><br></br>
				
				<?php
				for ($i = 0; $i < count($changelogs) && $i < 5; $i++) {
					?>
					<li><?php echo getClock($changelogs[$i]['time'], true, true); ?></li>
					<div>	<?php echo $changelogs[$i]['text']; ?></div>
					
					<?php
				}
				?><div><a href="changelog.php" style="color:gray">See more changelogs</a></div><br></br></div>
			<br></br><br></br>
		...
		for ($i = $current; $i < $current + $config['news_per_page']; $i++) {
			if (isset($news[$i])) {
				...
				<div style="border: 1px solid gray"><br></br>
					...
					<div class="block-title"></div><br></br></div>
				<?php
			}
		}

		echo '<br></br><center><select name="newspage" onchange="location = this.options[this.selectedIndex].value;" style="width:100px;background:#18171c;color:white">';
